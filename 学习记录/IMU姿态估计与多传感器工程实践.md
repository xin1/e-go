...
> 文档性质：原理说明、实现指南与调试记录  
> 硬件背景：ESP32-S3/Arduino，JY901P 系列，多 IMU I²C 总线  
> 更新日期：2026-08-14
...
其中 $\boldsymbol\omega_m$ 是测量值，$\boldsymbol\omega$ 是真实角速度，$\mathbf b_g$ 是随温度和时间变化的零偏，$\mathbf n_g$ 是噪声。姿态需要对角速度积分，因此即使固定零偏很小，也会随时间形成明显角度漂移。
...
加速度计常见确定性误差包括零偏、尺度因子和轴不正交。六位置标定比单面置零更可靠：分别令传感器的 $\pm X$、$\pm Y$、$\pm Z$ 轴朝向重力方向，利用已知的 $\pm g$ 估计偏置和比例。
...
系统至少涉及传感器坐标系 $S$、肢体段坐标系 $B$ 和世界坐标系 $W$。安装外参 ${}^{B}\mathbf R_S$ 用于把每颗 IMU 的测量统一到对应肢体段：
...
1. 若点积小于 0，将其中一个四元数取反，以选择较短旋转路径；$\mathbf q$ 与 $-\mathbf q$ 表示同一姿态。
