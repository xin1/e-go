...
/*
Test on Arduino Uno. use HW-97 module connect WT901C485 sensor
wiring:
  Arduino Uno          HW-97            WT901C485
   5V/3.3V  <--------------------------->  VCC
   5V/3.3V  <----->     VCC
     GND    <--------------------------->  GND2
                        GND   <----->      GND1  
...
void loop() {
    ...
    {
      WitSerialDataIn(SensorSerial.read());
    }
...
static void CmdProcess(void)
{
	switch(s_cCmd)
	{
		...
		case 'B':	if(WitSetUartBaud(WIT_BAUD_115200) != WIT_HAL_OK) Serial.print("\r\nSet Baud Error\r\n");
              ...
              {
                 SensorSerial.begin(c_uiBaud[WIT_BAUD_115200]);
		...
		case 'b':	if(WitSetUartBaud(WIT_BAUD_9600) != WIT_HAL_OK) Serial.print("\r\nSet Baud Error\r\n");
              ...
              {
                SensorSerial.begin(c_uiBaud[WIT_BAUD_9600]);
